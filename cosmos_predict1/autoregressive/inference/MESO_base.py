...
def main(args):
    ...
    data = np.load("/public/home/sunhaofei/cosmos-predict1/workspace/Tokenizer_prep/MESO_20230428_29_normalized.npz")
    # print(data.files)
    inp_vid_tensor = torch.from_numpy(data['data'])  # 先转换为torch.Tensor
    inp_vid = inp_vid_tensor.unsqueeze(0) 
    print(f"Input video shape: {inp_vid.shape}")  # 应该是 (1, 3, 33, 640, 1024)

    # Generate video
    # log.info(f"Run with image or video path: {input_filename}")
    log.info(f"inp_vid.shapeh: {inp_vid.shape}")
    ...
    log.info(f"out_vid.shape: {out_vid.shape}")
    if out_vid is None:
        log.critical("Guardrail blocked base generation.")
        # continue

    # Save out_vid as npz
    output_file = os.path.join(args.input_image_or_video_path, f"{args.video_save_name}.npz")
    output_video = out_vid.float().cpu().detach().numpy() # 转换为numpy数组
    # print max and min values of out_vid
    print(f"Max value in out_vid: {output_video.max()}")
    print(f"Min value in out_vid: {output_video.min()}")  
    np.savez(output_file, data=output_video)
    log.info(f"Saved generated video to {output_file}")
    
    # out_vid_path = os.path.join(args.input_image_or_video_path, f"{args.video_save_name}.mp4")
