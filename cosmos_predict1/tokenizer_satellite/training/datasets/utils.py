...
def pad_channels_to_max(video_tensor: torch.Tensor, max_channels: int, pad_value: float = 0.0) -> torch.Tensor:
    """Pad video tensor channels to a maximum number of channels.
    
    This function implements Scheme A: Unified Training with Maximum Channel Count.
    For videos with fewer channels than max_channels, it pads with zeros (or specified value).
    For videos with more channels than max_channels, it truncates (should not happen in practice).
    ...
    Returns:
        Padded video tensor with exactly max_channels channels
    """
    # Handle different tensor formats
    if len(video_tensor.shape) == 4:
        # Determine if format is [C, T, H, W] or [T, C, H, W]
        # Check if first dimension is likely channels (usually smaller) or time (usually larger)
        if video_tensor.shape[0] < video_tensor.shape[1]:
            # Likely [C, T, H, W] format
            current_channels = video_tensor.shape[0]
            if current_channels < max_channels:
                # Pad channels: [C, T, H, W] -> [max_C, T, H, W]
                pad_shape = (max_channels - current_channels, video_tensor.shape[1], 
                           video_tensor.shape[2], video_tensor.shape[3])
                padding = torch.full(pad_shape, pad_value, dtype=video_tensor.dtype, device=video_tensor.device)
                video_tensor = torch.cat([video_tensor, padding], dim=0)
            elif current_channels > max_channels:
                # Truncate channels (should not happen in practice)
                video_tensor = video_tensor[:max_channels, :, :, :]
        else:
            # Likely [T, C, H, W] format
            current_channels = video_tensor.shape[1]
            if current_channels < max_channels:
                # Pad channels: [T, C, H, W] -> [T, max_C, H, W]
                pad_shape = (video_tensor.shape[0], max_channels - current_channels,
                           video_tensor.shape[2], video_tensor.shape[3])
                padding = torch.full(pad_shape, pad_value, dtype=video_tensor.dtype, device=video_tensor.device)
        ...
        raise ValueError(f"Expected 4D tensor [C, T, H, W] or [T, C, H, W], got shape {video_tensor.shape}")
