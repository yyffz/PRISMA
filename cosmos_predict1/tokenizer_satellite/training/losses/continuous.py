...
from cosmos_predict1.tokenizer_satellite.training.losses import ReduceMode
from cosmos_predict1.tokenizer_satellite.training.losses.lpips import LPIPS
...
class PerceptualLoss(LPIPS):
    ...
    def __init__(self, config):
        vgg_weights_path = getattr(config, 'vgg_weights_path', None)
        super(PerceptualLoss, self).__init__(config.checkpoint_activations, vgg_weights_path=vgg_weights_path)
    ...
    def forward(self, inputs, output_batch, iteration):
        ...
        # Check if all loss weights are zero - if so, skip computation entirely
        # This avoids errors when input has different channel count than expected (e.g., 14 channels vs 3 for RGB)
        lpips_weight = self.lpips_schedule(iteration)
        gram_weight = self.gram_schedule(iteration) if self.gram_enabled else 0.0
        corr_weight = self.corr_schedule(iteration) if self.corr_enabled else 0.0
        
        if lpips_weight == 0.0 and gram_weight == 0.0 and corr_weight == 0.0:
            # All weights are zero, return zero losses without computation
            # Create zero tensors with appropriate shape (e.g., [B, 1, H, W] or [B, 1, T, H, W])
            # This ensures the loss dict has expected keys with zero values
            dummy_shape = list(input_images.shape)
            if input_images.ndim == 5: # [B, C, T, H, W]
                dummy_shape[1] = 1 # Dummy channel
            else: # [C, H, W] or [C, T, H, W] or [B, C, H, W]
                if input_images.ndim == 4: # [B, C, H, W]
                    dummy_shape[1] = 1 # Dummy channel
                else: # [C, H, W] or [C, T, H, W]
                    dummy_shape[0] = 1 # Dummy channel
            
            output_dict["lpips"] = torch.zeros(dummy_shape, dtype=input_images.dtype, device=input_images.device)
            if self.gram_enabled:
                output_dict["gram"] = torch.zeros(dummy_shape, dtype=input_images.dtype, device=input_images.device)
            if self.corr_enabled:
                output_dict["corr"] = torch.zeros(dummy_shape, dtype=input_images.dtype, device=input_images.device)
            return output_dict
        ...
        # ScalingLayer only supports 3-channel RGB images
        # If input has more channels, use only the first 3 channels for LPIPS computation
        # After time2batch, input_images is always 4D: [B, C, H, W]
        # Channel dimension is at index 1
        num_channels = input_images.shape[1]
        if num_channels != 3:
            # Use only first 3 channels for LPIPS (RGB)
            input_images_rgb = input_images[:, :3, :, :]
            reconstructions_rgb = reconstructions[:, :3, :, :]
        else:
            input_images_rgb = input_images
            reconstructions_rgb = reconstructions

        in0_input, in1_input = (self.scaling_layer(input_images_rgb), self.scaling_layer(reconstructions_rgb))
...
class VideoConsistencyLoss(torch.nn.Module):
    ...
    def shuffle(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        # print(f'self.num_windows:{self.num_windows}')
    ...
    def forward(self, inputs, output_batch, iteration) -> dict[str, torch.Tensor]:
        ...
        # print(f'videos[:, :-1, :, self.step :, :, :].shape:{videos[:, :-1, :, self.step :, :, :].shape}')
        # print(f'videos[:, 1:, :, : -self.step, :, :].shape:{videos[:, 1:, :, : -self.step, :, :].shape}')
        ...
        # print(f'T_latent:{T_latent}')
        temporal_rate = self.num_frames // T_latent
        # print(f'temporal_rate:{temporal_rate}')

        spatial_rate = (H // H_latent) * (W // W_latent)
        # print(f'self.step:{self.step}')
        step_latent = self.step // temporal_rate
        # print(f'step_latent:{step_latent}')
        # print(f'latents[:, :-1, :, step_latent:, :, :].shape:{latents[:, :-1, :, step_latent:, :, :].shape}')
        # print(f'latents[:, 1:, :, :-step_latent, :, :].shape:{latents[:, 1:, :, :-step_latent, :, :].shape}')
