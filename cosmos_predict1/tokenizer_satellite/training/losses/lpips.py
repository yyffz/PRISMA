...
# _TORCH_HOME = os.getenv("TORCH_HOME", os.path.join(os.path.expanduser("~"), ".cache", "torch"))
# 使用用户可访问的路径，优先使用环境变量，否则使用项目目录
# _TORCH_HOME = os.getenv("TORCH_HOME", "/public/home/sunhaofei/cosmos-predict1/checkpoints")
_TORCH_HOME = "/public/home/sunhaofei/cosmos-predict1/checkpoints"
...
class LPIPS(nn.Module):
    def __init__(self, checkpoint_activations: bool = False, vgg_weights_path: str = None):
        super().__init__()
        self.scaling_layer = ScalingLayer()
        self.chns = [64, 128, 256, 512, 512]  # vg16 features
        self.net = vgg16(pretrained=True, requires_grad=False, checkpoint_activations=checkpoint_activations, vgg_weights_path=vgg_weights_path)
...
class vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True, checkpoint_activations: bool = False, vgg_weights_path: str = None):
        super(vgg16, self).__init__()
        
        # If vgg_weights_path is specified, load directly from file to avoid any network requests
        if vgg_weights_path is not None:
            print(f'load from {vgg_weights_path}')
            if not os.path.exists(vgg_weights_path):
                raise FileNotFoundError(f"VGG16 weights file not found: {vgg_weights_path}")
            
            # In distributed training, only rank 0 loads the file
            ...
            # Use default torchvision loading (may trigger download)
            # In distributed training, only rank 0 should load/download the model weights
            # to avoid network conflicts (BrokenPipeError) even when file exists
            if pretrained and dist.is_initialized():
                # Non-rank0 processes wait for rank0 to load/download
                if not is_rank0():
                    dist.barrier()
                
                # Rank0 loads/downloads the weights
                if is_rank0():
                    vgg_pretrained_features = models.vgg16(pretrained=pretrained).features
                    dist.barrier()
                else:
                    # Other processes wait for rank0 to complete, then load
                    vgg_pretrained_features = models.vgg16(pretrained=pretrained).features
            else:
                # Not in distributed mode or not using pretrained weights
                vgg_pretrained_features = models.vgg16(pretrained=pretrained).features
