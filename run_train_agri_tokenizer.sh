...
# AGRI Tokenizer 训练任务提交脚本
# 使用方法: ./run_train_agri_tokenizer.sh [选项]
...
# 默认参数
DATA_PATTERN="/public/share/users/sunhaofei/yyf_data/AGRI_processed/preprocessed_video/*_agri_video_*.npz"
USE_MASK_CHANNEL=true  # true=10通道（9个AGRI通道+1个mask通道），false=9通道（仅AGRI通道）
NUM_GPUS=1  # 使用的GPU数量
CUDA_VISIBLE_DEVICES_DEFAULT="0"  # 默认只使用物理GPU 0
BATCH_SIZE=1  # 批次大小；单卡80G下使用32 latent通道时建议从1开始
...
LATENT_CHANNELS=32  # tokenizer latent/z 通道数；预训练权重为16时会循环复制初始化到32
RESUME_FROM_CHECKPOINT=""  # 断点续训：指定checkpoint路径（如：checkpoints/satellite_tokenizer/agri/agri_training_2026-01-05_12-49-28/checkpoints/iter_000240000.pt），如果为空则自动从latest_checkpoint.txt恢复
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --cuda_visible_devices)
            CUDA_VISIBLE_DEVICES_DEFAULT="$2"
            shift 2
            ;;
        --batch_size)
            BATCH_SIZE="$2"
        ...
        -h|--help)
            ...
            echo ""
            echo "可选参数:"
            echo "  --data_pattern PATTERN      数据文件路径模式 (默认: /public/share/users/sunhaofei/AGRI_processed/preprocessed/*_agri_video_*.npz)"
            ...
            echo "  --cuda_visible_devices STR  可见GPU ID (默认: 0)"
            echo "  --batch_size INT            批次大小 (默认: 1)"
            ...
            echo "  --latent_channels INT       tokenizer latent/z通道数 (默认: 32)"
            ...
            echo ""
            echo "  # 自定义数据路径和训练参数"
            echo "  $0 --data_pattern=\"datasets/AGRI_processed/preprocessed/*_agri_video_*.npz\" \\"
...
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/train_agri_${TIMESTAMP}.log"

echo "=========================================="
echo "AGRI Tokenizer 训练任务"
echo "=========================================="
echo "项目根目录: $PROJECT_ROOT"
echo "数据路径: $DATA_PATTERN"
echo "使用mask通道: $USE_MASK_FLAG ($NUM_CHANNELS 通道)"
echo "时间步数: $NUM_TIME_STEPS"
echo "GPU数量: $NUM_GPUS"
echo "可见GPU: $CUDA_VISIBLE_DEVICES_DEFAULT"
...
echo "Latent通道数: $LATENT_CHANNELS"
...
TRAIN_CMD="${TRAIN_CMD} --"
TRAIN_CMD="${TRAIN_CMD} experiment=agri_training"
...
TRAIN_CMD="${TRAIN_CMD} model.config.network.latent_channels=${LATENT_CHANNELS}"
TRAIN_CMD="${TRAIN_CMD} model.config.network.z_channels=${LATENT_CHANNELS}"
TRAIN_CMD="${TRAIN_CMD} model.config.channel_init_strategy=all_pretrained"
...
# 根据是否使用mask通道添加相应参数
# 注意：AGRI通道数已从15个改为9个（只使用中红外和热红外通道）
# 需要显式设置通道数，因为默认配置是16通道
if [ "$USE_MASK_FLAG" = "false" ]; then
    ...
    TRAIN_CMD="${TRAIN_CMD} checkpoint.jit.input_shape=[1,9,${NUM_TIME_STEPS},256,256]"
else
    ...
    TRAIN_CMD="${TRAIN_CMD} checkpoint.jit.input_shape=[1,10,${NUM_TIME_STEPS},256,256]"
fi
